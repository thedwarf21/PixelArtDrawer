...
class RS_Dialog extends HTMLDivElement {
  ...
  constructor(id, title, bgClassList, containerClassList, classList, showCloseBtn, urlHtmlContent, onAfterContentLoad) {
    ...
    let header_caption = document.createElement("DIV");
    header_caption.classList.add("rs-modal-text-title");
    header_caption.innerHTML = title;
  ...
  closeModal() {
    ...
    // On attend la fin de l'animation pour supprimer le nœud (ou son wrapper, si la popup est dans un shadow DOM)
    setTimeout(()=> {
      if (this.wrapper)
        this.wrapper.remove();
      else popup.parentNode.remove(); 
    }, 500);
  }
 ...
 * dans le template                                               *
 ******************************************************************/
class RSWCLDialog extends HTMLBaseElement {
  ...
  childrenAvailableCallback() {
    ...
    RS_WCL.styleShadow(shadow, COMPONENTS_CSS_PATH + '/rs_dialog.css');
...
function RS_Alert(msg, titre, lbl_btn, callback) {
  ...
  popup.wrapper = RS_WCL.addShadowElementWithStyle(popup, "rs_dialog.css");
}
